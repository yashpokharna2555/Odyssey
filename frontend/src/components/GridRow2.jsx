...
export default function GridRow2() {
  const TILE_COUNT = 5; // total tiles in the row
  const IMAGE_COUNT = 2; // how many ImgTiles per row (balanced look)

  // 🎲 Randomize positions once per render
  const tileTypes = useMemo(() => {
    const positions = Array.from({ length: TILE_COUNT }, () => "blank");

    // randomly pick IMAGE_COUNT unique positions
